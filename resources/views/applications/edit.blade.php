                ...
                <div class="p-6 max-w-md">
                    <form method="POST" action="{{ route('applications.update', $application) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <select id="equipment_type_id" name="equipment_type_id" class="block mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600">
                                ...
                                @foreach($equipmentTypes as $et)
                                    <option value="{{ $et->id }}" @selected(old('equipment_type_id', $application->equipment_type_id) == $et->id)>{{ $et->name }}</option>
                                @endforeach
                            </select>
                            <x-text-input id="equipment_name" class="block mt-2 w-full" type="text" name="equipment_name" :value="old('equipment_name', $application->equipment_name)" placeholder="или введите вручную" />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="quantity" class="block mt-1 w-full" type="number" name="quantity" :value="old('quantity', $application->quantity)" min="1" required />
                            <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="desired_delivery_date" value="Желаемая дата поставки" />
                            <x-text-input id="desired_delivery_date" class="block mt-1 w-full" type="date" name="desired_delivery_date" :value="old('desired_delivery_date', $application->desired_delivery_date?->format('Y-m-d'))" required />
