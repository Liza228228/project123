@php // шаблон страницы
@endphp
@if($application->changeJournalEntries->contains(fn ($entry) => $entry->application_item_id !== null))
    <section class="space-y-3" aria-labelledby="show-section-change-journal">
        ...
        <p class="text-xs text-black/70 dark:text-white/70">
            Записи о правках заявки: изменённое оборудование и комментарий автора.
        </p>
            ...
            <table class="min-w-full divide-y divide-stone-200 text-sm dark:divide-stone-700">
                <thead class="bg-stone-50/90 dark:bg-stone-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-2 text-left font-medium text-black dark:text-white">Оборудование</th>
                ...
                <tbody class="divide-y divide-stone-200 bg-white dark:divide-stone-700 dark:bg-stone-950/30">
                    @foreach($application->changeJournalEntries->filter(fn ($entry) => $entry->application_item_id !== null) as $entry)
                        <tr>
                            ...
                            <td class="px-3 py-2 text-black dark:text-white">{{ $entry->equipmentLineLabel() ?? '—' }}</td>
                            <td class="max-w-xs px-3 py-2 text-xs text-amber-900/90 dark:text-amber-100/85">
                                {{ filled(trim((string) ($entry->reason ?? ''))) ? $entry->reason : '—' }}
                            </td>
                            <td class="whitespace-nowrap px-3 py-2 text-xs text-black/80 dark:text-white/80">
                                @if($entry->user)
