<x-app-layout>
    ...
    <div class="py-2 sm:py-8 md:py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="post" action="{{ route('applications.report.footers.update', $footer) }}" class="bg-white dark:bg-orange-950 rounded-lg border border-orange-200 dark:border-orange-800 shadow-sm p-4 sm:p-6 space-y-6">
                @csrf
                @method('PUT')
                @include('applications.report.footers._fields', ['settings' => $settings, 'fontOptions' => $fontOptions, 'nameValue' => old('name', $footer->name)])
