@php
    ...
    $s = $settings;
    $rawDateText = (string) old('settings.date_text', $s['date_text'] ?? '');
    $headerDateTextValue = \Illuminate\Support\Carbon::now()->format('Y-m-d');
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', trim($rawDateText)) === 1) {
        $headerDateTextValue = trim($rawDateText);
    } elseif (trim($rawDateText) !== '') {
        try {
            $headerDateTextValue = \Illuminate\Support\Carbon::parse($rawDateText)->format('Y-m-d');
        } catch (\Throwable $e) {
            $headerDateTextValue = \Illuminate\Support\Carbon::now()->format('Y-m-d');
        }
    }
    ...
    <div class="space-y-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="settings[date_text]" value="{{ $headerDateTextValue }}"
                    class="w-full rounded-lg border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-900/40 text-black dark:text-white text-sm shadow-sm focus:border-stone-500 focus:ring-stone-500">
            </div>
            <div>
...
<script>
    (function () {
        ...
        // Initial heuristic for edit mode: try match role by exact position text
