    ...
    <div class="mx-auto max-w-3xl px-0 py-2 max-sm:-mx-4 sm:px-6 sm:py-8 md:py-10 lg:px-8">
        <div class="app-form-card">
            ...
            <div class="px-4 py-5 sm:p-8">
                <form id="application-create-form" method="POST" action="{{ route('applications.store') }}" enctype="multipart/form-data" class="max-sm:pb-40 space-y-8 sm:space-y-10">
                    @csrf
                    ...
                    <section class="space-y-4" aria-labelledby="create-section-equipment">
                        ...
                        <div id="equipment-items" class="space-y-4">
                            @foreach($items as $idx => $item)
                                    ...
                                    <div class="equipment-row equipment-row--custom app-equipment-card">
                                        ...
                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-end">
                                            <div class="md:col-span-12">
                                                ...
                                                <input type="text" name="items[{{ $idx }}][equipment_name]" value="{{ $item['equipment_name'] ?? '' }}" placeholder="" maxlength="{{ $equipmentNameMax }}" class="custom-equipment-input app-input" />
                                ...
                                @else
                                    @php
                                        $selectedEq = ($item['equipment_id'] ?? '') !== '' ? $equipment->firstWhere('id', (int) $item['equipment_id']) : null;
                    ...
                    <div class="app-form-actions-mobile">
                        ...
                        <x-input-error :messages="$errors->get('submit_action')" class="w-full sm:order-last sm:basis-full" />
                        @if ($usesDraftSubmitFlow ?? false)
                            <button type="submit" name="submit_action" value="save"
                                    class="ui-btn ui-btn--secondary ui-btn--lg w-full text-base disabled:opacity-60 disabled:cursor-not-allowed sm:w-auto"
                                    @disabled($subdivisions->isEmpty())>
                                Сохранить
                            </button>
                            @if (Auth::user()->hasRoleId(7))
                                <button type="submit" name="submit_action" value="submit_for_management"
                                        class="ui-btn ui-btn--primary ui-btn--lg w-full text-base disabled:opacity-60 disabled:cursor-not-allowed sm:w-auto"
                                        @disabled($subdivisions->isEmpty())>
