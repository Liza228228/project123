    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 w-full min-w-0">
            ...
            <h2 class="font-semibold text-xl text-black dark:text-white leading-tight tracking-tight min-w-0 break-words">
                Назначение подразделений мастерам
            </h2>
        </div>
    </x-slot>
        ...
        <div class="app-form-card">
            <div class="px-4 py-5 sm:p-8">
                <div class="mb-6 border-b border-stone-100 pb-4 dark:border-stone-800 sm:pb-6">
                    <h3 class="app-section-title">Мастера участка</h3>
                    <p class="mt-2 text-xs text-stone-500 dark:text-stone-400 max-w-2xl">
                        Выберите мастера, чтобы назначить или изменить подразделения.
                    </p>
                </div>

                <form method="get" action="{{ route('foreman-subdivisions.assignments') }}" class="mb-6 space-y-3" data-auto-submit="filter">
                    <div class="app-filter-panel">
                        <div class="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
                            <div class="min-w-0">
                                <label for="foremen-assignments-q" class="app-form-label">Поиск</label>
                                <div class="relative">
                            ...
                            <div class="w-full lg:w-40">
                                <label for="foremen-assignments-per-page" class="app-form-label">На странице</label>
                                <select
                                    name="per_page"
                                    id="foremen-assignments-per-page"
                                    class="app-input"
                                >
                            ...
                            <div class="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-end lg:pb-0.5">
                                @if($search !== '' || $perPage !== $defaultPerPage)
                                    <a href="{{ route('foreman-subdivisions.assignments') }}" class="ui-btn ui-btn--secondary w-full min-h-[3rem] content-center text-center lg:min-h-0 lg:w-auto">Сбросить</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                @if($foremen->isEmpty())
                    @if($search !== '')
                        <p class="py-8 text-center text-sm text-stone-600 dark:text-stone-400">
                            ...
                            <a href="{{ route('foreman-subdivisions.assignments') }}" class="font-medium text-orange-700 underline decoration-orange-300 underline-offset-2 hover:text-orange-800 dark:text-orange-300 dark:hover:text-orange-200">Показать всех</a>
                        </p>
                    @else
                        <p class="py-8 text-center text-sm text-stone-600 dark:text-stone-400">Мастера участка не найдены.</p>
                    @endif
                @else
                    <div class="md:hidden space-y-4">
                        @foreach($foremen as $foreman)
                            <article class="app-equipment-card space-y-3">
                                <div>
                                    <p class="text-[10px] font-semibold uppercase tracking-wide text-orange-800/90 dark:text-orange-200/90">Мастер участка</p>
                                    <p class="mt-1 text-sm font-medium text-stone-900 dark:text-stone-100 break-words">{{ $foreman->surname }} {{ $foreman->name }} {{ $foreman->patronymic }}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium uppercase tracking-wide text-stone-500 dark:text-stone-400">Почта</p>
                                    <p class="mt-0.5 text-sm text-stone-700 dark:text-stone-300 break-all">{{ $foreman->email }}</p>
                                </div>
                                <div>
                                ...
                                <a href="{{ route('foreman-subdivisions.edit', $foreman) }}" class="ui-btn ui-btn--primary ui-btn--lg flex w-full justify-center min-h-[3rem] py-3 [touch-action:manipulation]">
                    ...
                    <div class="hidden md:block app-table-shell">
                        <table class="min-w-full">
                            <thead>
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-stone-600 dark:text-stone-400">Мастер участка</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-stone-600 dark:text-stone-400">Почта</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-stone-600 dark:text-stone-400">Назначенные подразделения</th>
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-8 text-center text-sm text-stone-600 dark:text-stone-400">Мастера участка не найдены.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
