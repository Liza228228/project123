<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between w-full min-w-0">
            <h2 class="font-semibold text-xl text-black dark:text-white leading-tight">Редактирование шапки</h2>
    ...
    <div class="py-2 sm:py-8 md:py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="post" action="{{ route('applications.report.headers.update', $header) }}" class="bg-white dark:bg-orange-950 rounded-lg border border-orange-200 dark:border-orange-800 shadow-sm p-4 sm:p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('applications.report.headers.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-black dark:text-white rounded-lg border border-orange-300 dark:border-orange-600">Отмена</a>
                </div>
            </form>
        </div>
    </div>
